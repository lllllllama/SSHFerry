...
const messages: Record<AppLanguage, Record<string, MessageValue>> = {
  zh: {
    ...
    'workspace.middlePanelDescription': '可在本地浏览和远端会话之间切换。',
    'workspace.middleSession': '显示会话',
    'workspace.middleRemoteEmptyTitle': '当前没有可显示的远端会话',
    'workspace.middleRemoteEmptyBody': '先在左侧打开至少一个远端会话，或切回本地面板。',
    'workspace.secondaryRemoteEmptyTitle': '当前没有其他远端会话',
    'workspace.secondaryRemoteEmptyBody': '中间面板已占用一个远端会话。继续打开其他会话后，会显示在右侧。',
    ...
    'log.title': '日志中心',
    'log.description': '查看后端实时日志，便于排查连接、传输和调度问题。',
    'log.pageDescription': '独立日志页使用终端样式展示完整日志流，更适合长时间排查和滚动查看。',
    'log.summary': ({ total }) => '缓冲 ' + total + ' 条',
    'log.clear': '清空日志',
    'log.cleared': '日志已清空',
    'log.autoScroll': '自动滚动',
    'log.emptyTitle': '当前没有日志',
    'log.emptyBody': '等待后端事件、任务调度或连接操作产生新的日志。',
    ...
    'socket.logPollFailed': '日志轮询失败',
    'socket.logChannelErrorTitle': '日志通道返回错误',
    'socket.logWebsocketError': '日志 WebSocket 连接异常',
  },
  en: {
    'app.title': 'SSHFerry',
    ...
    'workspace.middlePanelDescription': 'Switch between the local browser and an open remote session.',
    'workspace.middleSession': 'Displayed Session',
    'workspace.middleRemoteEmptyTitle': 'No remote session is available here',
    'workspace.middleRemoteEmptyBody': 'Open at least one remote session from the left, or switch back to the local panel.',
    ...
    'log.description': 'Inspect realtime backend logs for connection, transfer, and scheduler troubleshooting.',
    'log.pageDescription': 'The dedicated log page uses a terminal-style stream, which is better for long sessions and larger log reviews.',
